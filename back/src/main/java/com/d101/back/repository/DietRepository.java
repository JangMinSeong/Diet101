...
public interface DietRepository extends JpaRepository<Meal, Long> {
	...
	@Query("SELECT FUNCTION('month', meal.time) AS month, SUM(meal.totalCalorie) AS totalCalorie, SUM(meal.totalCarbohydrate) AS totalCarbohydrate, SUM(meal.totalFat) AS totalFat, SUM(meal.totalProtein) AS totalProtein FROM Meal meal WHERE meal.user.email = :email AND FUNCTION('year', meal.time) = FUNCTION('year', CURRENT_DATE) GROUP BY FUNCTION('month', meal.time)")
